...
resource "google_compute_target_pool" "k8s_lbtartgetpool" {
  ...
  instances = [
    "${var.region}-${var.zone_map[0]}/${google_compute_instance.k8s_controller[0].name}",
    "${var.region}-${var.zone_map[1]}/${google_compute_instance.k8s_controller[1].name}",
    "${var.region}-${var.zone_map[2]}/${google_compute_instance.k8s_controller[2].name}",
  ]
